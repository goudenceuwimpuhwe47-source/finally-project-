      ...
      <div className="container mx-auto px-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-8">
          {features.map((feature, index) => (
            <Card key={index} className="group hover:shadow-lg transition-all duration-300 border-gray-700 shadow-md hover:-translate-y-1 bg-gray-800">
              <CardContent className="p-6 sm:p-8">
                <div className="bg-blue-900 w-12 h-12 sm:w-16 sm:h-16 rounded-xl flex items-center justify-center mb-4 sm:mb-6 group-hover:bg-blue-600 transition-colors duration-300 mx-auto sm:mx-0">
                  <feature.icon className="h-6 w-6 sm:h-8 sm:w-8 text-blue-400 group-hover:text-white transition-colors duration-300" />
                </div>
                <h3 className="text-lg sm:text-xl font-semibold text-white mb-2 sm:mb-3 text-center sm:text-left">{feature.title}</h3>
                <p className="text-gray-300 leading-relaxed text-center sm:text-left">{feature.description}</p>
              </CardContent>
            </Card>
          ))}
        </div>
